   ...
   public Checkout() {
	...
	btn2.addActionListener(this);
	 ...
	 try  
			...
			FileInputStream fis=new FileInputStream("F:/Desktop 24-09-2022/java project/hotel/data/new_customer.txt");   
    ...
    model.addRow(new Object[]{valueToshowOntable[0],valueToshowOntable[1], valueToshowOntable[2], valueToshowOntable[3]});		
		...
		ImageIcon i1= new ImageIcon(ClassLoader.getSystemResource("image\\check.jpg"));
			...
			if(e.getSource()==btn1)
		 {
			   int selectedRow = table.getSelectedRow();
                if (selectedRow != -1) {
                    ...
                    writeDataToFile("F:/Desktop 24-09-2022/java project/hotel/data/new_customer.txt");
                } else {
        ...
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(filename))) {
             for (int i = 0; i < model.getRowCount(); i++) {
                ...
                for (int j = 0; j < model.getColumnCount(); j++) {
					...
					if(j==1){
					row.append("NID Number :"+ model.getValueAt(i, j));
						row.append("\n");
					}
					if(j==2){
					row.append("Address :"+ model.getValueAt(i, j));
						row.append("\n");
					}
					if(j==3){
